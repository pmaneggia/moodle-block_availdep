define("block_availdep/graphManipulation",["exports"],(function(_exports){function computePredecessors(node){return node.depend.c.filter((function(x){return"completion"===x.type}))}function handleDanglingReferencesInNode(depend,ids){if(depend.op){var danglingReferenceFound=!1;return depend.c.forEach((function(d){var found=handleDanglingReferencesInNode(d,ids);danglingReferenceFound||(danglingReferenceFound=found)})),danglingReferenceFound}return"completion"===depend.type&&!ids.includes(depend.cm)&&(depend.cm=-2,!0)}Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.fixDanglingReferences=function(dependencies){var ids=dependencies.map((function(x){return x.id})).concat([-1]),danglingReferenceFound=!1;dependencies.forEach((function(node){if(node.depend){var found=handleDanglingReferencesInNode(node.depend,ids);danglingReferenceFound||(danglingReferenceFound=found)}})),danglingReferenceFound&&dependencies.push({id:-2,name:"missing",depend:null,predecessor:null})},_exports.removeDisconnectedNodes=
/**
