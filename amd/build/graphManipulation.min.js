define("block_availdep/graphManipulation",["exports"],(function(_exports){function computePredecessors(node){return node.depend.c.filter((x=>"completion"===x.type))}function handleDanglingReferencesInNode(depend,ids){if(depend.op){let danglingReferenceFound=!1;return depend.c.forEach((d=>{let found=handleDanglingReferencesInNode(d,ids);danglingReferenceFound||(danglingReferenceFound=found)})),danglingReferenceFound}return"completion"===depend.type&&!ids.includes(depend.cm)&&(depend.cm=-2,!0)}Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.fixDanglingReferences=function(dependencies){const ids=dependencies.map((x=>x.id)).concat([-1]);let danglingReferenceFound=!1;dependencies.forEach((node=>{if(node.depend){let found=handleDanglingReferencesInNode(node.depend,ids);danglingReferenceFound||(danglingReferenceFound=found)}})),danglingReferenceFound&&dependencies.push({id:-2,name:"missing",depend:null,predecessor:null})},_exports.removeDisconnectedNodes=
/**
